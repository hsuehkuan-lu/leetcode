...
class Solution {
public:
    bool dfs(vector<vector<int>> &graphs, int cur, vector<bool> &visited, int destination) {
        if(cur == destination) return graphs[cur].empty();
        if(graphs[cur].empty()) return false;
        for(auto e: graphs[cur]) {
            if(visited[e]) return false;
            visited[e] = true;
            if(!dfs(graphs, e, visited, destination))
                return false;
            visited[e] = false;
        }
        return true;
    ...
    bool leadsToDestination(int n, vector<vector<int>>& edges, int source, int destination) {
        ...
        for(auto &edge : edges) {
            graphs[edge.front()].push_back(edge.back());
        }
        vector<bool> visited(n, false);
        return dfs(graphs, source, visited, destination);
    }
};
...
int main() {
    ...
    cout << solution.leadsToDestination(4, v, 0, 3);
    return 0;
}