...
static int x = [](){cin.tie(0); cout.tie(0); ios::sync_with_stdio(false); return 0;}();
class Solution {
public:
    int connectSticks(vector<int>& sticks) {
        if(sticks.size() <= 1) return 0;
        priority_queue<int, vector<int>, greater<>> min_heap(sticks.begin(), sticks.end());
        int total = 0;
        while(min_heap.size() > 1) {
            int sum = min_heap.top();
            min_heap.pop();
            sum += min_heap.top();
            min_heap.pop();
            total += sum;
